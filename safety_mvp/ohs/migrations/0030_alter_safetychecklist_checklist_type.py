# Generated by Django 4.2.8 on 2026-04-28 12:13

from django.db import migrations, models


def migrate_drill_rig_to_drilling_machine_surface(apps, schema_editor):
    SafetyChecklist = apps.get_model("ohs", "SafetyChecklist")
    SafetyChecklist.objects.filter(checklist_type="Drill Rig").update(
        checklist_type="Drilling Machine Surface"
    )


class Migration(migrations.Migration):
    dependencies = [
        ("ohs", "0029_safetychecklist_area_name_and_more"),
    ]

    operations = [
        migrations.RunPython(
            migrate_drill_rig_to_drilling_machine_surface,
            migrations.RunPython.noop,
        ),
        migrations.AlterField(
            model_name="safetychecklist",
            name="checklist_type",
            field=models.CharField(
                choices=[
                    ("Daily", "Daily"),
                    ("Weekly", "Weekly"),
                    ("Monthly", "Monthly"),
                    ("LV Vehicle", "LV Vehicle Checklist"),
                    ("Lighting Tower", "Lighting Tower Inspection Checklist"),
                    ("Drilling Machine Surface", "Drilling Machine Surface Checklist"),
                    ("Environmental", "Environmental Checklist"),
                    ("Generator", "Generator Checklist"),
                    ("Other Operational", "Other Operational Checklist"),
                ],
                max_length=40,
            ),
        ),
    ]
